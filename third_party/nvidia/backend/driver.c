...
static PyObject *fillTMADescriptorTiled(PyObject *self, PyObject *args) {
  ...
  // CUDA 13.2 and 13.3 forward-compatibility userspace drivers. They report 13020
  // and 13030 respectively, even when the host kernel driver is still affected.
  // Gating only through 13020 therefore regressed MXFP4 MoE under NGC 26.06:
  // persistent UTMALDG.5D kernels hit deterministic illegal memory accesses on B200.
  //
  // Keep the official <128KB condition below and extend the compatibility-driver
  // gate through CUDA 13.3. Clearing bit 85 is NVIDIA's documented workaround.
